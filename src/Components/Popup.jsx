...
const Popup = ({ isOpen, onClose, documentName }) => {
  ...
  const [selectedOption, setSelectedOption] = useState('Only people with access can see');
  ...
  return (
    ...
    >
      ...
      />
              <p style={{marginBottom:0 , paddingBottom:2}} className="text-md font-medium ">People with access </p>
      ...
      <div>
        <span className="text-sm">{selectedOption}</span>
          ...
          <BiDownArrow className={`ml-2 ${dropdownOpen ? 'transform rotate-180' : ''}`} />
        </button>
        {dropdownOpen && (
          <ul style={{zIndex:'999999'}} className="absolute bg-white border border-gray-200 rounded-md mt-2">
            <li
              className={`px-4 hover:bg-gray-100 cursor-pointer ${
                selectedOption === 'Only people with access can see' ? 'font-semibold' : 'Only people with access can see'
              }`}
              onClick={() => handleOptionClick('Only people with access can see')}
            >
              Only people with access can see
            </li>
            <li
              className={`px-4 hover:bg-gray-100 cursor-pointer ${
                selectedOption === 'Anyone with the link' ? 'font-semibold' : 'Anyone with the link'
              }`}
  ...
  sx={{
    ...
    borderEndEndRadius:4,
                      ...
                      > */}
                          <Button sx={{textTransform: 'none'}} variant="outlined"><div className="flex"> <LinkIcon /> Copy link</div></Button>
 {/* Copy link
                      </Button> */}
                  </div>
                  <div>
                       <Button sx={{textTransform: 'none', backgroundColor:"blue"}}  variant="contained">Done</Button>
                  </div>
                  </div>
      </div>
    </Modal>
