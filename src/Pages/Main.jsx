...
// const AddFontSize = () => {
//   return (
//       <div className='text-black border-r-2 border-gray-300 '>
//         <IoIosAdd/>
//      </div>
//   )
// }
...
const MainToolBAr = () => {
  ...
  const toolIcons = [
    ...
    // { icon: <HiOutlineMinusSm />, tooltip: 'Decrease font size' },
    { icon: <FontSizer />, tooltip: 'font size' },
    { icon: <StyeComponent />, tooltip: 'style' },
    // { icon: <AddFontSize />, tooltip: 'Increase font size' },
    ...
    {icon:<><Button 
      id="composition-button"
        ...
        aria-haspopup="true"
      sx={{ minWidth: 0, margin: 0, padding: 0, marginLeft: 0.5, color: 'black' }}> <MdChecklist sx={{ padding: 0, margin: 0 }} className="text-xl ml-2 mt-1" /></Button> <CheckList /></>, tooltip: 'CheckList'
    },
    {icon:<><Button 
      id="composition-button"
        aria-controls={open ? "composition-menu" : undefined}
        aria-expanded={open ? "true" : undefined}
        aria-haspopup="true"
      sx={{ minWidth: 0, margin: 0, padding: 0, marginLeft: 0.25, color: 'black' }}> <MdFormatListBulleted sx={{ padding: 0, margin: 0 }} className="text-xl ml-1 mt-1 " /></Button> <BulletList /></>, tooltip: 'BulletList'
    },
    {icon:<><Button 
      id="composition-button"
        aria-controls={open ? "composition-menu" : undefined}
        aria-expanded={open ? "true" : undefined}
        aria-haspopup="true"
        sx={{ minWidth: 0 , margin:0 , padding:0 , marginLeft:0.25 , color:'black' }}> <BsListOl sx={{ padding: 0, margin: 0 }} className="text-xl ml-1 mt-1" /></Button> <NumberList/></> , tooltip:'NumberList'},
    {
      icon:
      <IconButton sx={{minWidth:0 , padding:0 }}>  <CgFormatIndentDecrease className='text-2xl mt-1' /></IconButton>, tooltip: 'Ident Decrease'
    },
    {
      icon:
      <IconButton sx={{minWidth:0 , padding:0 }}>  <CgFormatIndentIncrease className='text-2xl mt-1' /></IconButton>, tooltip: 'Ident Increase'
    },
    {
      icon:
      <IconButton sx={{minWidth:0 , padding:0 }}>  <RiFormatClear className='text-2xl mt-1' /></IconButton>, tooltip: 'Format Clear'
    },
     ...
     <>
      ...
      <div className={`flex items-center bg-slate-200 space-x-0 h-[45px] rounded-2xl mr-4 ml-4 ${isUp ? 'mt-1' : 'mt-6'} ${isUp ? 'fixed top-0 w-full z-10' : ''}`}>
        {toolIcons.map((tool, index) => (
          index >= 5 ? 
            index === 19 ? <><div className='flex justify-end'>{tool.icon}</div></>:
          <>
              {tool.icon}
           </>
         :
          <IconButton sx={{ padding: 0.5, margin: 0 }} key={index} className="rounded-full hover:bg-gray-200 w-[35px] " title={tool.tooltip}>
            {tool.icon}
          </IconButton>
        ))}
